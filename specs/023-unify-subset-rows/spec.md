...
1. **Given** a main set followed by a pyramid sub-set and another surviving row, **When** the pyramid row's menu delete action reaches the section's delete request path, **Then** only that pyramid row is removed, the undo snackbar is armed, and undo restores its values and original relative position.
2. **Given** a main set followed by a drop-set sub-set and another surviving row, **When** the drop-set row is deleted through trailing full swipe, **Then** the swipe invokes the same section delete request path as the menu, only that drop-set row is removed, and undo restores it.
...
- A refused deletion must not arm undo or announce success.
...
- **FR-003**: Both menu deletion and trailing full swipe MUST call the same production section-level delete request path used by normal rows.
- **FR-004**: The section-level delete request path MUST continue to resolve confirmation through SetDeletionPolicy, execute deletion through WorkoutSetManager.deleteSet, and arm the existing SetDeletionUndoController only after a successful deletion.
...
- **FR-006**: Each sub-set delete entry MUST render a hit target of at least 44 by 44 points and expose a localized VoiceOver label containing parent number, current sub-set type and delete intent.
...
- **FR-008**: Verification MUST cover pyramid and drop-set production-route deletion, undo restoration, sibling/parent isolation, parent cascade regression, shared row configuration, hit targets, VoiceOver copy and four iPhone 16 Simulator appearance/mode combinations.
...
- **SC-001**: Automated tests independently delete and undo one pyramid and one drop-set through the same production request handler used by ActiveExerciseSection, with zero parent or sibling loss.
...
- **SC-003**: Every tested delete affordance measures at least 44 by 44 points and every sub-set delete accessibility label includes parent number, sub-set type and delete intent.
