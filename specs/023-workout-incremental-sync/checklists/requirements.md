...
- [x] Stage 0A pins local `794624a…`, parent/tracking/remote/PR `f5690f…`, exact local-commit and three dirty-file blobs/numstats, owner `113937f4…`, Draft state, and no active/orphan owner; Stage 0B excludes only the expected Fullstack run and rechecks the composite. Both fail closed without reset/clean/stash/checkout/revert/amend/rebase.
- [x] Phase 0C captures three D behavior REDs and records the post-RED test blob, eliminates five service access deltas while cache/post-RED tests remain exact, then removes the cache tracking/order seam with service/tests fixed; service equals `4b1f809…`, cumulative service diff zero, and `794624a…` history remains.
