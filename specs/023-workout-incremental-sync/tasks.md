...
**Stage 0 readiness**: before editing, record a clean preserved-workspace packet at exact rejected SHA `f5690f1461a6cb07504d7f6e945220cb5213b2fb`: local/tracking/remote/PR equality, Draft/no-auto-merge state, no active owner, two-file base diff, obsolete source-pattern inventory, and unchanged test blob `9bdc7a5fd2b528b8f9273395480d9efa02625616`. If any value differs, stop without reset/clean/recheckout and return the divergence to Team Lead.

Each stage starts only after the previous exit. For its next tracer bullet, record the named test, exact RED command/nonzero exit/observable failure before the relevant production change, then the GREEN command/exit, replacement diff, and prior-stage regression result. Accumulate the ledger for the single final handoff; a progress comment, package-green claim, or audit claim without byte evidence is not an exit.
...
1. **Stage A — semantic reconciliation foundation**
   ...
   - Entry: Stage 0 packet captured and the next public-cache behavior demonstrably RED.
   - Exit: every listed behavior has sequential RED/GREEN evidence; focused Stage A suite passes; diff remains allowlisted.
...
5. **Stage E — remaining deterministic acceptance matrix**
   ...
   - Entry: Stage D exit plus the next named uncovered matrix behavior RED.
