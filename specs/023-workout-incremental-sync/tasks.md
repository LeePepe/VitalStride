...
The three named public-cache RED tracers are the sole executable-evidence exception: run them in the metadata-pinned workspace during 0C before source scrub. All remaining executable evidence runs there only after 0C exits. Record actual `swift build`; compiler status proves no behavior. If RED, repair only `HealthDataCache.swift` and never widen excluded service witnesses. Do not pre-implement B settlement, C reset-drain, or remaining D publication outcomes before their mandatory evidence.
...
6. **Stage F — package and diff gate**
   ...
   - Exit: commit/push a new SHA above `794624a…`; prove ancestry, local package gate, local/tracking/remote/PR equality, Draft/no-auto-merge, exact-two-file diff, and clean pattern audit; Fullstack authors the exact-SHA Multica review request. Team Lead waits for same-head PASS before handing the unchanged head to PR Manager.
...
- Fullstack leaves Stage F only after local package pass, exact-two-file diff, clean pattern audit, preserved ancestry, Draft/no-auto-merge, publish/equality proof, and its exact-SHA Multica review request.
- After same-head Multica PASS, Team Lead hands the unchanged head to PR Manager; PR Manager owns required CI/build/test/lint/hook supervision and shipping readiness. Any head change returns to Fullstack and exact-SHA review.
