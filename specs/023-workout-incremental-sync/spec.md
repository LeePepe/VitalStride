...
9. **Given** a fetch is cancelled, invalidated, or fails, **when** it completes late or throws, **then** it does not replace the last valid cache entry.
...
- **FR-009**: Workout-specific invalidation generation MUST prevent cancelled or late work from repopulating or overwriting a newer cache entry.
- **FR-010**: `workoutData(in:)`, `refreshWorkouts(in:)`, and the existing direct `fetchWorkouts(dateRange:)` entry point MUST remain source-compatible.
...
- **FR-014**: Provider failure, cancellation, or invalidation MUST preserve the last valid entry and surface existing error behavior to the caller.
...
- A more explicit provider request/result seam may be added only with source-compatible defaults and behavioral tests.

## Success Criteria

- **SC-001**: All nine acceptance scenarios are deterministic unit tests and pass without HealthKit hardware.
...
- **SC-005**: Invalidation and late-result tests prove that stale work cannot repopulate the cache.
