...
- A legacy snapshot-only fallback or a prepared baseline/change result with no candidate checkpoint publishes an entry with checkpoint absent and invokes no checkpoint acceptance; the records/coverage/provenance transition remains valid.
...
- **FR-011**: The precise prepared-fetch capability MUST remain package-internal and MUST NOT add public witness requirements. Existing public provider conformers and tests MUST remain source-compatible through anchor-free snapshot-only fallback behavior. A legacy fallback result has no checkpoint, publishes a checkpoint-absent snapshot entry, and invokes no checkpoint acceptance; HealthKitService and cache tests may adopt the package-internal capability for precise snapshot/delta semantics.
...
- **FR-015**: An anchor-advancing provider fetch MUST be available only through the cache-facing prepare/accept seam and may return an opaque pending checkpoint without persisting it. When a current prepared baseline/change supplies a candidate, the cache MUST publish one immutable entry containing it and synchronously invoke provider acceptance for the same candidate in one actor turn with no intervening suspension or waiter completion. When the candidate is absent—including a nil-checkpoint prepared result or legacy snapshot fallback—the cache MUST publish checkpoint-absent state and invoke no checkpoint acceptance. The existing `Void` acceptance is not a persistence receipt: silent no-advance leaves the durable anchor behind and MUST be handled by idempotent replay. A rejected result and the direct snapshot entry point MUST neither publish a candidate checkpoint nor invoke acceptance.
...
- **Workout cache entry**: One immutable, sorted record set plus its coverage, snapshot provenance, and optional opaque cache-accepted candidate checkpoint. The checkpoint is present only when an accepted prepared baseline/change supplied a candidate; it is absent for legacy snapshot fallback, nil-checkpoint prepared results, and anchor-free explicit-range snapshots.
