...
- **Publish a reviewed planning descendant, freeze a dedicated PR base at it, and later fast-forward PR #418 separately**: selected. Let `H0=cb78fe8c70071c11f91bf400b00ef14b7812e771`, `P` be the fresh planning-only reviewed descendant, and `H1` be the T002 implementation head. The dedicated base ref `agent/planner-lead/my1490-pr-base` points exactly to `P`, so the actual T002 PR surface `P...H1` contains only the two authorized test files. After review, PR #418 can separately fast-forward from `H0` to `H1` without losing either reviewed ancestor.
