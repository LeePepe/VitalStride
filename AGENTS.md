...
- 本个人仓库：`LeePepe/VitalStride`（ID `1256865689`）；Git 与 gh 的预期认证账号均为 `LeePepe`。提交署名不是认证。
- 当前 GitHub remote：`github`。新 clone/worktree/Multica checkout 须重新核对实际 fetch/push URL；
...
- gh 使用 `$HOME/.config/github-identity/profiles/LeePepe` 的独立无凭据 profile，复用已有 keyring。
  本文所有 gh 命令（含其他章节示例）均加下列调用前缀；仓库命令显式 `--repo LeePepe/VitalStride`，
...
```bash
# 原生 gh 调用形状；先检查 /user，再同前缀检查 repos/LeePepe/VitalStride 的 id/full_name/permissions。
...
git config --local --replace-all credential.https://github.com/LeePepe/VitalStride.git.helper ''
git config --local --add credential.https://github.com/LeePepe/VitalStride.git.helper \
  '!env -u GH_TOKEN -u GITHUB_TOKEN GH_CONFIG_DIR="$HOME/.config/github-identity/profiles/LeePepe" gh auth git-credential'
git config --local credential.https://github.com/LeePepe/VitalStride.git.username LeePepe
```
